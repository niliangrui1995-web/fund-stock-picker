...
出海钱眼是一个静态基金持仓穿透工具，用公开基金持仓数据反查海外股票被哪些基金重仓。用户输入美股、港股、日股、韩股或其他海外股票的名称/代码后，页面会展示场外基金和场内 ETF / LOF 等品种的持仓排序；若底层明细披露了海外个股杠杆 ETF / ETP / ETN，也会单独归入“间接 / 杠杆 ETF 暴露”。首页也提供 AI 战报热点入口，便于从 LITE、COHR、GLW 等光通信上游标的，以及 005930、MU、SNDK、000660 等 AI 存储和半导体高频标的直接跳到持仓穿透结果。
...
- `LITE`、`COHR`、`GLW`、`005930`、`MU`、`SNDK`、`000660`：查看 AI 光通信 / 存储热点标的对应的基金持仓穿透结果。
...
- `code` 使用站内股票代码口径，例如 `AMD`、`LITE`、`COHR`、`GLW`、`000660`、`005930`、`MU`、`SNDK`；脚本会确认当前季度前端 JSON 里存在同一 `code`。
...
- 线上首页 `/` 和首页脚本资产是否已经包含 `config/ai-battle-hotspots.json` 中配置的 AI 战报热点入口，且这些标的能从线上前端数据文件匹配出卡片所需的基金覆盖和最高占比。
- 旧 `/stocks/<code>/` 深链样例会真实请求 `https://fund.niliangrui.cloud/stocks/AMD/`、`LITE`、`COHR`、`GLW`、`000660`、`005930`、`MU`、`SNDK`，确认最终回到 `/?stock=<code>` 并能保留正确股票上下文。
