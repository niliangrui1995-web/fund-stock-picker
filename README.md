...
发布到 Cloudflare Pages 后核对线上产物：

```powershell
npm run verify-live-release
```

该命令会直接请求 `https://fund.niliangrui.cloud/seo/quarter-release-check.json` 和清单里记录的前端数据文件，并与本地 `config/fund-quarter.json`、`public/seo/quarter-release-check.json` 核对。全部通过时说明线上站点已经换成当前季度产物；失败时会列出不一致字段并以非零退出码结束。
