...
出海钱眼是一个静态基金持仓穿透工具，用公开基金持仓数据反查海外股票被哪些基金重仓。用户输入美股、港股、日股、韩股或其他海外股票的名称/代码后，页面会展示场外基金和场内 ETF / LOF 等品种的持仓排序；若底层明细披露了海外个股杠杆 ETF / ETP / ETN，也会单独归入“间接 / 杠杆 ETF 暴露”。首页也提供 AI 战报热点入口，便于从 005930、MU、SNDK、SK海力士等 AI 存储和韩国半导体高频标的直接跳到持仓穿透结果。
...
- `005930`、`MU`、`SNDK`、`000660`：查看 AI 存储和韩国半导体热点标的对应的基金持仓穿透结果。
...
- `code` 使用站内股票代码口径，例如 `AMD`、`LITE`、`COHR`、`000660`、`005930`、`MU`、`SNDK`；脚本会确认当前季度前端 JSON 里存在同一 `code`。
